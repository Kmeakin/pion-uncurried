...
impl ParseError {
    pub fn to_diagnostic(&self, file: FileId) -> Diagnostic<FileId> {
        match self {
            Self::TooLong(TooLong(len)) => Diagnostic::error()
                .with_message("Parse error: input is too long")
            ...
            Self::UnrecognizedToken(range, unexpected, expected) => Diagnostic::error()
            ...
            Self::ExtraToken(range, unexpected) => Diagnostic::error()
                .with_message(format!("parse error: unexpected token (`{unexpected}`)"))
                .with_labels(vec![Label::primary(file, *range)]),
        }
    }
